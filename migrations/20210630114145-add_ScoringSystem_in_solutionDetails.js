module.exports = {
  async up(db) {
    ...
    global.migrationMsg = "Add scoringSystem in Solution Details inside Project and ProjectTemplateTask";
    ...
    for (let pointerToProject = 0; pointerToProject < chunkOfProjectDocument.length; pointerToProject++) {
      ...
      await Promise.all(
        projectDocuments.map(async eachProjectDocument => {
          ...
          let solution = await db.collection('solutions').find({externalId: eachProjectDocument.solutionExternalId}).project({scoringSystem: 1}).toArray();
          ...
          if(solution && solution.length > 0){
            ...
            for (let pointerToTaskArray = 0; pointerToTaskArray < tasks.length; pointerToTaskArray++) {
              ...
              if(currentTask.type == "observation"){
                ...
                if(!("scoringSystem" in solutionDetails)){

                  solutionDetails.scoringSystem = solutionData.scoringSystem ? solutionData.scoringSystem : "";
      ...
      await Promise.all(
        templateTaskDocuments.map(async eachTemplateTaskDocument => {
            ...
            if(eachTemplateTaskDocument.type == "observation"){
              ...
              let solution = await db.collection('solutions').find({externalId: solutionId}).project({scoringSystem: 1}).toArray();
